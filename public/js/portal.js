...
authForm.addEventListener('submit', async (event) => {
  ...
  const { error } = await supabase.auth.signInWithOtp({ email, options: { emailRedirectTo: `${window.location.origin}/portal.html` } });
...
async function loadProfile() {
  const response = await fetch('/.netlify/functions/customer-profile', { headers: { Authorization: `Bearer ${session.access_token}` } });
