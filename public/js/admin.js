...
authForm.addEventListener('submit', async (event) => {
  ...
  const { error } = await supabase.auth.signInWithOtp({ email, options: { emailRedirectTo: `${window.location.origin}/admin.html` } });
...
async function loadAdminData(token) {
  ...
  const response = await fetch('/.netlify/functions/admin-data', { headers: { Authorization: `Bearer ${token}` } });
...
function renderTickets(tickets, token) {
  ...
  tickets.forEach((ticket) => {
    ...
    card.querySelector('button').addEventListener('click', async () => {
      await fetch('/.netlify/functions/resolve-ticket', {
        method: 'POST',
